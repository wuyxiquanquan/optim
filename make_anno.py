...
def make_anno(path):
    ...
    # Saving this as npy
    save_heatmaps = np.array([seg_heatmap, heatmap])
    # np.save(f'seg/{id_name}.npy', save_heatmaps)
...
def sss():
    ...
    for i in [1, 2, 4, 8, 16, 32, 64]:
        t1 = time.time()
        with Pool(i) as p:
            with tqdm(p.imap_unordered(make_anno, asf), total=len(asf)) as pbar:
                for x in pbar:
                    pass
        t2 = time.time() 
